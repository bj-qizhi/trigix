...
const NODE_TYPE_LIST: { type: NodeType; label: string; color: string; desc: string; category: string }[] = [
  ...
  { type: 'sftp',         label: 'SFTP',          color: 'var(--node-awss3)',  desc: 'SFTP over SSH (pure-Rust). Config: host, port, host_key_fingerprint (SHA-256), username, password or private_key (+passphrase), operation (list/download/upload/delete), path, content (base64).', category: 'Integration' },
  { type: 'ssh',          label: 'SSH',           color: 'var(--node-code)',  desc: 'Run a command over SSH. Config: host, port, host_key_fingerprint (SHA-256), username, password or private_key (+passphrase), command. Returns {stdout, stderr, exit_status}.', category: 'Integration' },
  ...
  { type: 'openai_compat', label: 'OpenAI-Compatible LLM', color: 'var(--node-openai)',  desc: 'Any OpenAI-compatible chat endpoint. Config: provider (grok/ollama/deepseek/qwen/zhipu/moonshot/doubao/hunyuan) for a preset endpoint+model, or base_url for a custom one; api_key, model, prompt_template, system_prompt, max_tokens, temperature. Returns {content, model, usage}.', category: 'AI' },
...
const NODE_ZH: Record<string, { labelZh: string; descZh: string }> = {
  ...
  openai_compat: { labelZh: 'OpenAI 兼容大模型', descZh: '调用任意 OpenAI 兼容端点：选 provider 预设（grok/ollama/deepseek/qwen/zhipu/moonshot/doubao/hunyuan）或填 base_url，返回内容与用量。' },
  ...
  sftp:         { labelZh: 'SFTP',        descZh: 'SFTP over SSH（SHA-256 主机密钥指纹，密码或私钥）：列目录/下载/上传/删除。' },
  ssh:          { labelZh: 'SSH',         descZh: 'SSH 执行命令（SHA-256 主机密钥指纹，密码或私钥），返回 stdout/stderr/exit。' },
  ...
  return (
    <div className="app">
      ...
      <header className="topbar">
        ...
        {workflow && (
          <TagEditor workflow={workflow} workflowId={workflowId} zh={zh} toast={toast} onUpdate={setWorkflow} />
        )}
        {version && (
      ...
      <div className="editor" style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '4px 14px', background: 'var(--panel)', borderBottom: '1px solid var(--border)', fontSize: 11, color: 'var(--muted)' }}>
          <GraphStatsBadges nodes={nodes} edges={edges} zh={zh} />
          {isDirty && (
