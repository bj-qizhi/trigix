...
const NODE_DESCRIPTIONS: Partial<Record<NodeType, { en: string; zh: string }>> = {
  ...
  sqlserver: { en: 'Run a SQL query against Microsoft SQL Server (pure-Rust TDS).', zh: '对 Microsoft SQL Server 执行 SQL（纯 Rust TDS）。' },
  ftp:       { en: 'FTP / FTPS: list/download/upload/delete files (base64 payloads).', zh: 'FTP / FTPS：列目录/下载/上传/删除（base64 载荷）。' },
  sftp:      { en: 'SFTP over SSH with a pinned SHA-256 host-key fingerprint: list/download/upload/delete.', zh: '通过固定 SHA-256 主机密钥指纹的 SFTP：列目录/下载/上传/删除。' },
  ssh:       { en: 'Run a command over SSH with a pinned SHA-256 host-key fingerprint; returns stdout/stderr/exit.', zh: '通过固定 SHA-256 主机密钥指纹的 SSH 执行命令，返回 stdout/stderr/exit。' },
...
const NODE_OUTPUTS: Partial<Record<NodeType, string[]>> = {
  ...
  // schema — left undeclared (dynamic) so the UI doesn't advertise dead fields;
  // the "last run output" tree still lets you click real paths.
  approval:     ['approved', 'comment'],
  condition:    ['result', 'field'],
  switch:       ['value', 'matched_case', 'matched'],
  map:          ['count', 'items'],
  filter:       ['count', 'items'],
  aggregate:    ['result', 'count'],
  sort:         ['count', 'items'],
  dedupe:       ['items', 'count', 'removed_count'],
  split:        ['parts', 'count'],
  csv:          ['rows', 'count', 'headers'],
  regex:        ['matched', 'full_match', 'groups', 'source'],
  ...
  for_each:     ['results', 'succeeded', 'failed', 'total'],
  slack:        ['ok', 'text'],
  email:        ['ok', 'to', 'subject'],
  ...
  return (
    <div className="config-panel">
      ...
      >
        ...
        {nt === 'sqlserver'       && <SqlserverConfig       config={config} set={set} str={str} num={num} />}
