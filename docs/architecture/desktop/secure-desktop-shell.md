...
`apps/desktop/scripts/prepare-sidecar.ps1` builds the Host for the same explicit target triple and configuration as the shell, then places the target-suffixed binary under `src-tauri/binaries`. Tauri validates that sidecar at bundle time and installs it under the unsuffixed executable name expected by the runtime. Windows CI compiles the native fixture, signs that fixture with an ephemeral qualification-only Authenticode identity, verifies it before native tests, and creates an unsigned NSIS qualification bundle. The temporary fixture identity is non-exportable, is trusted only through an in-memory custom chain without modifying the operating-system Root or Trusted Publisher stores, and is removed after bounded public signature evidence is uploaded. Production Authenticode signing, publisher identity, timestamping, and installer reputation remain release gates; neither the fixture signature nor the qualification bundle may be represented as a production-signed installer.
