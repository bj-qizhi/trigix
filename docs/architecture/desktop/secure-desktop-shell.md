...
`apps/desktop/scripts/prepare-sidecar.ps1` builds the Host for the same explicit target triple and configuration as the shell, then places the target-suffixed binary under `src-tauri/binaries`. Tauri validates that sidecar at bundle time and installs it under the unsuffixed executable name expected by the runtime. Windows CI compiles the native fixture, signs that fixture with an ephemeral qualification-only Authenticode identity, verifies it before native tests, and creates an NSIS qualification bundle using a separate short-lived self-issued development identity. Both private keys are non-exportable. Verification uses an isolated custom chain without modifying the operating-system Root or Trusted Publisher stores, records artifact digests and the expected signer, and deletes the identities after bounded public evidence is uploaded. macOS CI likewise creates a one-day self-issued code-signing identity in an ephemeral keychain, signs the Universal application and DMG, verifies the exact identity and both architecture slices, confirms that public Gatekeeper assessment rejects the development identity, and destroys the keychain after evidence upload. Production Authenticode signing, Apple Developer ID notarization, publisher identity, timestamping, and installer reputation remain release gates; neither test identity nor any qualification bundle may be represented as a production-signed installer.
