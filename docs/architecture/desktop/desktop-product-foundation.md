...
Keyboard and pointer authoring remain selector-targeted. A keyboard action names one inspected window, one canonical key from the protocol vocabulary, and a deduplicated set of Control, Alt, Shift, or Meta modifiers. A pointer action names one inspected element, an enumerated button, and a click count of one or two. The browser cannot submit screen coordinates, scan codes, timing sequences, or arbitrary input packets. Both actions are high risk, require a current command-specific Approval, and are dispatched only to a Device that advertises the corresponding input capability plus UI Automation inspection.

While an author test is non-terminal, the editor displays the selected Device, action kind, and authoritative command state together with an immediate Stop control. Stop calls the authenticated tenant-scoped Platform cancellation endpoint; it never contacts the Device directly. The Platform persists the cancellation and emits the typed cancellation event to the active Device session. A completion race is resolved by reading the terminal command state, so repeated user intent cannot turn cancellation into a retry or a second side effect.

Medium- and high-risk commands now enter a durable `waiting_approval` state without an Approval grant and are not offered to the Device. The tenant administrator queue returns only command, Execution, Workflow, Device, action class, risk, fixed rationale, requester, and lease timestamps; it never returns the typed text, selector contents, credentials, screen content, or the raw command. Approval atomically writes an actor-attributed grant bounded by the existing command lease, changes the command to `queued`, and only then sends it to the current Device connection. Rejection is terminal. A repeated, expired, terminal, or cross-Tenant decision fails without creating a second side effect, while cancellation and timeout also apply during `waiting_approval`.

The Web operations view combines Workflow Approval nodes with the separate Desktop command queue and requires an explicit review-and-confirm step for each Desktop decision. Risk and the one-command/one-lease boundary are visible before confirmation. The same view can list and export Execution-scoped evidence metadata through the authenticated tenant boundary. Export contains no encrypted screenshot payload, and the table exposes only policy-safe selector strategy, application identity, outcome, redaction count, and retention deadline. The flow uses semantic headings, labelled regions, keyboard-focusable controls, an announced queue status, and no motion-dependent interaction.

The native Desktop shell provides complete English and Simplified Chinese copy for runtime, pairing, safety, notice, and failure states. The keyboard-operable locale control persists only the locale preference and updates the document language; it does not change protocol or Device identity state. Pairing phase transitions caused by an operator action move focus to the new phase heading, while periodic refreshes never move focus. Status updates and actionable failures use separate polite-status and alert regions, and every bounded failure offers a safe retry that repeats only the corresponding read or idempotent user intent. Local credential removal is protected by a modal confirmation and a single-operation guard.

The shell polls only while visible and performs an immediate refresh when the window becomes visible again. This avoids hidden-window churn without extending authority or caching sensitive state. Native controls retain visible keyboard focus, forced-colors support, responsive reflow, and motion only when the operating system has not requested reduced motion. Static shell contract tests run in the Rust and Windows lanes so removal of localization, focus, confirmation, visibility, or recovery boundaries fails qualification.
...
Keyboard input resolves one window and requires it to remain the foreground window. The Host maps only the validated canonical key vocabulary to Windows virtual keys, presses modifiers in declared order, releases them in reverse order, and rejects partial `SendInput` delivery. Pointer input resolves one visible element, derives the center from the native control bounds inside the Host, moves the system pointer there, and emits only the approved button and bounded click count. No coordinate crosses the Web or Platform boundary. Both paths recheck the execution guard immediately before operating-system input and return only key/button class and counts, never captured keystrokes or surrounding UI content.
...
Voice capture is consent-gated. Only the Desktop shell's explicit Start action may request browser microphone permission. While permission is pending or a track is live, the shell exposes a persistent accessible status and Stop control. Stop, hidden-window transitions, ended input tracks, session expiry, and page teardown release every local media track and realtime transport; denial or missing capture support fails closed.
...
Production realtime voice uses a Device-authenticated Platform bootstrap and direct WebRTC media path. The long-lived provider key is server-only; the provider's two-minute `ek_` setup secret exists only transiently in memory, and an established session is capped at 55 minutes. The provider Tool list is empty. The WebView accepts only fixed speech-boundary and final-transcription events and returns final text through Tauri and Device authentication to the existing Tenant privacy store. Latency, reconnect, interruption, failure, and stop telemetry contains only fixed categories and bounded numbers. The Device advertises `voice_conversation` only after both WebRTC and the provider data channel are open. See [ADR 0011](../../adr/0011-authenticated-realtime-voice-transport.md).
