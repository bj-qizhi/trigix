...
// Contact: managecode@gmail.com
    ...
    async fn starts_and_gets_execution_over_http() {
        ...
        let xform_out: serde_json::Value = serde_json::from_str(
            payload["node_results"][1]["output_json"].as_str().unwrap_or("{}"),
        ).unwrap();
    ...
    fn effective_tenant_id_jwt_always_wins() {
        ...
        assert_eq!(effective_tenant_id_with_flag(false, &claims_a, "tenant-b"), "tenant-a");
        // No JWT → fall back to supplied (dev mode only).
        assert_eq!(effective_tenant_id_with_flag(false, &None, "tenant-b"), "tenant-b");
    }
    ...
    async fn jwt_claims_injected_into_extensions() {
        ...
        use crate::auth::{Claims, sign_token};
    ...
    async fn workspace_crud_over_http() {
        ...
        let resp = app
            ...
            .oneshot(
                Request::builder()
                    ...
                    .uri("/v1/workspaces")
                    .header("content-type", "application/json")
                    .body(Body::from(json!({ "tenant_id": "tenant-1", "name": "Engineering" }).to_string()))
                    .unwrap(),
            )
            .await
            .unwrap();
        ...
        let resp = app
            ...
            .oneshot(
                Request::builder()
                    ...
                    .header("content-type", "application/json")
                    .body(Body::from(json!({ "tenant_id": "tenant-1", "name": "Backend" }).to_string()))
                    .unwrap(),
            )
            .await
            .unwrap();
        ...
        let resp = app
            ...
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri(format!("/v1/workspaces/{ws_id}/projects?tenant_id=tenant-1"))
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
    ...
    async fn system_info_returns_expected_fields() {
        ...
        assert!(info["features"].as_array().unwrap().iter().any(|f| f.as_str() == Some("jwt-auth")));
    }
    ...
    async fn search_returns_matching_workflows() {
        ...
        let resp = app.oneshot(
            Request::builder().method("GET").uri("/v1/search?q=SearchTarget&tenant_id=t1")
                .body(Body::empty()).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        let result: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        assert!(result["workflows"].as_array().map(|a| !a.is_empty()).unwrap_or(false));
    }
    ...
    async fn search_empty_query_returns_all() {
        let app = router();
        let resp = app.oneshot(
            Request::builder().method("GET").uri("/v1/search?q=&tenant_id=t1")
                .body(Body::empty()).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
    ...
    async fn node_type_analytics_returns_empty_initially() {
        let app = router();
        let resp = app.oneshot(
            Request::builder().method("GET").uri("/v1/analytics/node-types?tenant_id=t1")
                .body(Body::empty()).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
    ...
    fn rbac_viewer_blocked_from_write() {
        ...
        let viewer = Some(Claims {
            sub: "u".to_string(), tenant_id: "t".to_string(),
            workspace_id: "w".to_string(), project_id: "p".to_string(),
            exp: u64::MAX, role: Role::Viewer, ..Default::default()
        });
    ...
    fn rbac_editor_allowed_to_write() {
        ...
        let editor = Some(Claims {
            sub: "u".to_string(), tenant_id: "t".to_string(),
            workspace_id: "w".to_string(), project_id: "p".to_string(),
            exp: u64::MAX, role: Role::Editor, ..Default::default()
        });
    ...
    fn rbac_admin_can_perform_all_ops() {
        ...
        let admin = Some(Claims {
            sub: "u".to_string(), tenant_id: "t".to_string(),
            workspace_id: "w".to_string(), project_id: "p".to_string(),
            exp: u64::MAX, role: Role::Admin, ..Default::default()
        });
    ...
    async fn form_publish_and_get() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST")
                ...
                .header("content-type", "application/json")
                .body(Body::from(json!({"tenant_id":"t1","title":"My Form"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        ...
        for title in ["Form A", "Form B"] {
            let resp = app.clone().oneshot(
                Request::builder().method("POST")
                    .uri(format!("/v1/workflows/{wf_id}/publish-form"))
                    .header("content-type", "application/json")
                    .body(Body::from(json!({"tenant_id":"t1","title":title}).to_string())).unwrap()
            ).await.unwrap();
            assert_eq!(resp.status(), StatusCode::OK);
        }
    ...
    async fn test_case_create_list_update_delete() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("PATCH")
                .uri(format!("/v1/test-cases/{tc_id}"))
                .header("content-type", "application/json")
                .body(Body::from(json!({"name": "TC1 Updated"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        let updated: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
    ...
    async fn workflow_comments_crud() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST")
                ...
                .header("content-type", "application/json")
                .body(Body::from(json!({"tenant_id":"t1","author":"alice","body":"Hello world"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::CREATED);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("PATCH")
                ...
                .header("content-type", "application/json")
                .body(Body::from(json!({"tenant_id":"t1","body":"Updated body"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
    ...
    async fn workflow_lock_blocks_version_save() {
        ...
        // Lock it
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri(format!("/v1/workflows/{wf_id}/lock"))
                .header("content-type", "application/json")
                .body(Body::from(json!({"tenant_id":"t1"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri(format!("/v1/workflows/{wf_id}/versions"))
                .header("content-type", "application/json")
                .body(Body::from(json!({"tenant_id": "t1", "graph": min_graph}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::BAD_REQUEST);

        // Unlock it
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri(format!("/v1/workflows/{wf_id}/unlock"))
                .header("content-type", "application/json")
                .body(Body::from(json!({"tenant_id":"t1"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
    ...
    async fn event_subscriptions_crud() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("GET").uri("/v1/event-subscriptions?tenant_id=t1")
                .body(Body::empty()).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        ...
        // Delete
        let resp = app.clone().oneshot(
            Request::builder().method("DELETE").uri(format!("/v1/event-subscriptions/{sub_id}?tenant_id=t1"))
                .body(Body::empty()).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::NO_CONTENT);
    ...
    async fn openapi_json_returns_valid_spec() {
        let app = router();
        let resp = app.oneshot(
            Request::builder().method("GET").uri("/openapi.json")
                .body(Body::empty()).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
    ...
    async fn update_me_name() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/auth/register")
                .header("content-type", "application/json")
                .body(Body::from(json!({"email": "upme1@example.com", "password": "pass123"}).to_string())).unwrap()
        ).await.unwrap();
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        let reg: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        let token = reg["token"].as_str().unwrap().to_string();
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("PATCH").uri("/v1/auth/me")
                .header("content-type", "application/json")
                .header("authorization", format!("Bearer {token}"))
                .body(Body::from(json!({"name": "Updated Name"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        let me: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
    ...
    async fn update_me_password_success() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/auth/register")
                .header("content-type", "application/json")
                .body(Body::from(json!({"email": "upme2@example.com", "password": "oldpass"}).to_string())).unwrap()
        ).await.unwrap();
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        let reg: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        let token = reg["token"].as_str().unwrap().to_string();
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("PATCH").uri("/v1/auth/me")
                .header("content-type", "application/json")
                .header("authorization", format!("Bearer {token}"))
                .body(Body::from(json!({"current_password": "oldpass", "new_password": "newpass"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/auth/login")
                .header("content-type", "application/json")
                .body(Body::from(json!({"email": "upme2@example.com", "password": "newpass"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
    }
    ...
    async fn update_me_wrong_current_password_returns_401() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/auth/register")
                .header("content-type", "application/json")
                .body(Body::from(json!({"email": "upme3@example.com", "password": "rightpass"}).to_string())).unwrap()
        ).await.unwrap();
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        let reg: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        let token = reg["token"].as_str().unwrap().to_string();
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("PATCH").uri("/v1/auth/me")
                .header("content-type", "application/json")
                .header("authorization", format!("Bearer {token}"))
                .body(Body::from(json!({"current_password": "wrongpass", "new_password": "newpass"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::UNAUTHORIZED);
    }
    ...
    async fn admin_list_users() {
        ...
        for email in &["adm1@example.com", "adm2@example.com"] {
            app.clone().oneshot(
                Request::builder().method("POST").uri("/v1/auth/register")
                    .header("content-type", "application/json")
                    .body(Body::from(json!({"email": email, "password": "pw"}).to_string())).unwrap()
            ).await.unwrap();
        }

        // List users (no auth required in test mode)
        let resp = app.clone().oneshot(
            Request::builder().method("GET").uri("/v1/admin/users")
                .body(Body::empty()).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
    ...
    async fn admin_delete_user_success() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/auth/register")
                .header("content-type", "application/json")
                .body(Body::from(json!({"email": "del_target@example.com", "password": "pw"}).to_string())).unwrap()
        ).await.unwrap();
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        let reg: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/auth/register")
                .header("content-type", "application/json")
                .body(Body::from(json!({"email": "del_admin@example.com", "password": "pw"}).to_string())).unwrap()
        ).await.unwrap();
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("DELETE").uri(format!("/v1/admin/users/{target_id}"))
    ...
    async fn admin_cannot_delete_self() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/auth/register")
                .header("content-type", "application/json")
                .body(Body::from(json!({"email": "del_self@example.com", "password": "pw"}).to_string())).unwrap()
        ).await.unwrap();
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        let reg: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        let token = reg["token"].as_str().unwrap().to_string();
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("DELETE").uri(format!("/v1/admin/users/{user_id}"))
                .header("authorization", format!("Bearer {token}"))
                .body(Body::empty()).unwrap()
        ).await.unwrap();
    ...
    async fn invite_flow_create_and_accept() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/admin/invitations")
                .header("content-type", "application/json")
                .body(Body::from(json!({"email": "invited@example.com", "role": "editor"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::CREATED);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        ...
        // Look up the invite
        let resp = app.clone().oneshot(
            Request::builder().method("GET").uri(format!("/v1/invitations/{token}"))
                .body(Body::empty()).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        ...
        // Accept the invite
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/auth/accept-invite")
                .header("content-type", "application/json")
                .body(Body::from(json!({"token": token, "password": "securepass", "name": "Invited User"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::CREATED);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        let auth: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        assert_eq!(auth["user"]["email"], "invited@example.com");
        assert!(auth["token"].as_str().map(|t| t.len() > 10).unwrap_or(false));
    }
    ...
    async fn invite_cannot_be_used_twice() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/admin/invitations")
                .header("content-type", "application/json")
                .body(Body::from(json!({"email": "twice@example.com"}).to_string())).unwrap()
        ).await.unwrap();
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        let inv: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        let token = inv["token"].as_str().unwrap().to_string();
        ...
        app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/auth/accept-invite")
                .header("content-type", "application/json")
                .body(Body::from(json!({"token": &token, "password": "pw1"}).to_string())).unwrap()
        ).await.unwrap();
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/auth/accept-invite")
                .header("content-type", "application/json")
                .body(Body::from(json!({"token": &token, "password": "pw2"}).to_string())).unwrap()
        ).await.unwrap();
    ...
    async fn invite_list_and_revoke() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/admin/invitations")
                .header("content-type", "application/json")
                .body(Body::from(json!({"email": "revoke_me@example.com"}).to_string())).unwrap()
        ).await.unwrap();
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        let inv: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("GET").uri("/v1/admin/invitations")
                .body(Body::empty()).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        let list: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        assert!(list.as_array().unwrap().iter().any(|i| i["id"] == invite_id));

        // Revoke it
        let resp = app.clone().oneshot(
            Request::builder().method("DELETE").uri(format!("/v1/admin/invitations/{invite_id}"))
                .body(Body::empty()).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::NO_CONTENT);
    }
    ...
    async fn register_and_get_token(app: axum::Router, email: &str) -> String {
        let resp = app.oneshot(
            Request::builder().method("POST").uri("/v1/auth/register")
                .header("content-type", "application/json")
                .body(Body::from(json!({"email": email, "password": "pw1234"}).to_string())).unwrap()
        ).await.unwrap();
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        let body: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
    ...
    async fn create_and_list_org() {
        ...
        // Create org
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/orgs")
                .header("content-type", "application/json")
                .header("authorization", format!("Bearer {token}"))
                .body(Body::from(json!({"name": "Acme Corp"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::CREATED);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        ...
        // List orgs
        let resp = app.clone().oneshot(
            Request::builder().method("GET").uri("/v1/orgs")
                .header("authorization", format!("Bearer {token}"))
                .body(Body::empty()).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
    ...
    async fn add_member_and_switch_org() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/auth/register")
                .header("content-type", "application/json")
                .body(Body::from(json!({"email": "member@org.test", "password": "pw1234"}).to_string())).unwrap()
        ).await.unwrap();
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/orgs")
                ...
                .body(Body::from(json!({"name": "Beta Inc"}).to_string())).unwrap()
        ).await.unwrap();
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        let org: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        ...
        // Add member
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri(format!("/v1/orgs/{org_id}/members"))
                .header("content-type", "application/json")
                .header("authorization", format!("Bearer {owner_token}"))
                .body(Body::from(json!({"user_id": member_id, "role": "editor"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::CREATED);

        // Switch org as member
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri(format!("/v1/orgs/{org_id}/switch"))
    ...
    async fn non_member_cannot_switch_org() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/orgs")
                .header("content-type", "application/json")
                .header("authorization", format!("Bearer {owner_token}"))
                .body(Body::from(json!({"name": "Gamma LLC"}).to_string())).unwrap()
        ).await.unwrap();
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        let org: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
    ...
    async fn workflow_visibility_set_and_filter() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("PATCH").uri(format!("/v1/workflows/{wf_id}/visibility"))
                .header("content-type", "application/json")
                .body(Body::from(json!({"tenant_id": "vis-tenant", "visibility": "private"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        let updated: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("GET").uri("/v1/workflows?tenant_id=vis-tenant")
                .body(Body::empty()).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        ...
        assert!(found, "workflow with visibility=private still visible when created_by matches caller (both None)");

        // Set back to tenant
        let resp = app.clone().oneshot(
            Request::builder().method("PATCH").uri(format!("/v1/workflows/{wf_id}/visibility"))
                .header("content-type", "application/json")
                .body(Body::from(json!({"tenant_id": "vis-tenant", "visibility": "tenant"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
    ...
    async fn set_visibility_rejects_invalid_value() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("PATCH").uri(format!("/v1/workflows/{wf_id}/visibility"))
                .header("content-type", "application/json")
                .body(Body::from(json!({"tenant_id": "vis2-tenant", "visibility": "public"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
    }
    ...
    async fn forgot_password_always_returns_ok() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/auth/forgot-password")
                .header("content-type", "application/json")
                .body(Body::from(json!({"email": "nobody@example.com"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        let body: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
    ...
    async fn password_reset_full_flow() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/auth/register")
                .header("content-type", "application/json")
                .body(Body::from(json!({"email": email, "password": "oldpassword"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::CREATED);
        ...
        let token = body["token"].as_str().expect("token returned in dev mode").to_string();

        // Reset with new password
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/auth/reset-password")
                .header("content-type", "application/json")
                .body(Body::from(json!({"token": token, "new_password": "newpassword123"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/auth/login")
                .header("content-type", "application/json")
                .body(Body::from(json!({"email": email, "password": "newpassword123"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/auth/reset-password")
                .header("content-type", "application/json")
                .body(Body::from(json!({"token": token, "new_password": "anotherpassword"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::GONE);
    }
    ...
    async fn reset_password_rejects_short_password() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/auth/reset-password")
                .header("content-type", "application/json")
                .body(Body::from(json!({"token": "fake-token", "new_password": "abc"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
    }
    ...
    async fn email_verification_full_flow() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/auth/register")
                .header("content-type", "application/json")
                .body(Body::from(json!({"email": email, "password": "password123"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::CREATED);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        let body: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/auth/resend-verification")
                .header("content-type", "application/json")
                .body(Body::from(json!({"email": email}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/auth/verify-email")
                .header("content-type", "application/json")
                .body(Body::from(json!({"token": "no-such-token"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::NOT_FOUND);
    }
    ...
    async fn notification_prefs_get_and_update() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("GET").uri("/v1/auth/me/notifications")
                .header("authorization", format!("Bearer {}", token))
                .body(Body::empty()).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        ...
        // Update prefs
        let resp = app.clone().oneshot(
            Request::builder().method("PUT").uri("/v1/auth/me/notifications")
                .header("authorization", format!("Bearer {}", token))
                .header("content-type", "application/json")
                .body(Body::from(json!({"email_on_failure": true, "email_on_success": false}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
    ...
    async fn drain_mode_rejects_new_executions() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/executions")
                .header("content-type", "application/json")
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri("/v1/workflow-versions/v-1/executions")
                .header("content-type", "application/json")
                .body(Body::from(serde_json::json!({"tenant_id": "tenant-drain", "input_json": "{}"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::SERVICE_UNAVAILABLE);
    ...
    async fn billing_status_endpoint_returns_ok() {
        let app = router();
        let resp = app.oneshot(
            Request::builder().method("GET").uri("/v1/billing/status")
                .body(Body::empty()).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
    ...
    async fn webhook_replay_protection_rejects_stale_timestamp() {
        ...
        state.webhook_store.upsert(WebhookRecord {
            token: token.clone(),
    ...
    async fn rollback_creates_new_draft_version() {
        ...
        let resp = app.clone().oneshot(
            Request::builder().method("POST").uri(format!("/v1/workflows/{wf_id}/versions"))
                .header("content-type", "application/json")
                .body(Body::from(version_body.to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::CREATED);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        ...
        let resp = app.clone().oneshot(
            Request::builder()
                .method("POST")
                .uri(format!("/v1/workflows/{wf_id}/rollback/{version_id}?tenant_id=t-rollback"))
                .body(Body::empty()).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::CREATED);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        ...
        assert!(msg.contains("Rollback") || msg.contains("ollback"), "message should mention rollback: {msg}");
    }
    ...
    async fn mcp_manifest_returns_json() {
        let app = router();
        let resp = app.oneshot(
            Request::builder().method("GET").uri("/.well-known/mcp.json")
                .body(Body::empty()).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        let v: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        assert!(v.get("tools").and_then(|t| t.as_array()).map(|a| a.len() >= 2).unwrap_or(false));
    }
    ...
    async fn mcp_list_workflows_tool() {
        let app = router();
        let resp = app.oneshot(
            Request::builder().method("POST").uri("/v1/mcp/tools")
                .header("content-type", "application/json")
                .body(Body::from(json!({
                    "tool": "list_workflows",
                    "tenant_id": "tenant-1"
    ...
    async fn mcp_unknown_tool_returns_400() {
        ...
        let resp = app.oneshot(
            Request::builder().method("POST").uri("/v1/mcp/tools")
                .header("content-type", "application/json")
                .body(Body::from(json!({"tool": "nonexistent_tool"}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
    }
    ...
    async fn request_ids_are_unique_across_requests() {
        let r1 = router().oneshot(
            Request::builder().method("GET").uri("/healthz").body(Body::empty()).unwrap()
        ).await.unwrap();
        let r2 = router().oneshot(
            Request::builder().method("GET").uri("/healthz").body(Body::empty()).unwrap()
        ).await.unwrap();
        let id1 = r1.headers().get("x-request-id").unwrap().to_str().unwrap().to_string();
        let id2 = r2.headers().get("x-request-id").unwrap().to_str().unwrap().to_string();
    ...
    async fn webhook_delivery_recorded_on_trigger_success() {
        ...
        let resp = app.oneshot(
            Request::builder()
                ...
                .header("content-type", "application/json")
                .body(Body::from(json!({"x": 1}).to_string())).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::ACCEPTED);
        let deliveries = state.webhook_store.list_deliveries("test-delivery-token", 10).await;
    ...
    async fn webhook_delivery_recorded_on_quota_failure() {
        use crate::webhook::{WebhookRecord, WebhookStore};
        use crate::billing::{TenantQuota, BillingStore};
        let state = default_app_state();
        state.webhook_store.upsert(WebhookRecord {
        ...
        assert_eq!(resp.status(), StatusCode::PAYMENT_REQUIRED);
        let deliveries = state.webhook_store.list_deliveries("test-quota-tok", 10).await;
        assert_eq!(deliveries.len(), 1);
        assert!(!deliveries[0].success);
    ...
    async fn list_webhook_deliveries_endpoint() {
        use crate::webhook::{WebhookRecord, WebhookStore, WebhookDelivery};
        let state = default_app_state();
        state.webhook_store.upsert(WebhookRecord {
        ...
        state.webhook_store.record_delivery(WebhookDelivery {
            id: "del-1".to_string(),
    ...
    async fn notifications_list_and_mark_read() {
        ...
        let state = default_app_state();
        state.notification_store.create("tenant-1", None, "Test alert", "Something happened", "warning");
        let app = build_router(state);
        let resp = app.clone().oneshot(
            Request::builder()
                .method("GET")
    ...
    async fn webhook_condition_blocks_non_matching_payload() {
        ...
        let resp = app.clone().oneshot(
            Request::builder()
                ...
                .header("content-type", "application/json")
                .body(Body::from(r#"{"event":"login"}"#)).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::ACCEPTED);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        let body: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        assert!(body["error"].as_str().unwrap_or("").starts_with("filtered:"));
    }
    ...
    async fn webhook_condition_passes_matching_payload() {
        ...
        let resp = app.oneshot(
            Request::builder()
                ...
                .header("content-type", "application/json")
                .body(Body::from(r#"{"event":"purchase","amount":99.99}"#)).unwrap()
        ).await.unwrap();
        assert_eq!(resp.status(), StatusCode::ACCEPTED);
        let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        let body: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
    ...
    fn eval_condition_unit_tests() {
        ...
        let payload = serde_json::json!({ "event": "purchase", "amount": 150.0, "nested": { "x": true } });
    ...
    async fn credential_update_and_expiry() {
        ...
        let state = default_app_state();
        let cred = state.credential_store.create("tenant-1", "my-key", "secret-value").await.unwrap();
        ...
        let updated = state.credential_store.update(
            "tenant-1", &cred.id,
            Some("new-secret"),
        ...
        let expiring = state.credential_store.list_expiring("tenant-1", now + 7 * 86400).await.unwrap();
        assert_eq!(expiring.len(), 1);
        assert_eq!(expiring[0].id, cred.id);

        let not_expiring = state.credential_store.list_expiring("tenant-1", now + 86400).await.unwrap();
    ...
    async fn list_expiring_credentials_endpoint() {
        ...
        let now = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_secs();
        let c1 = state.credential_store.create("tenant-1", "soon-key-exp", "v").await.unwrap();
        state.credential_store.update("tenant-1", &c1.id, None, None, Some(Some(now + 5 * 86400))).await.unwrap();
        let c2 = state.credential_store.create("tenant-1", "far-key-exp", "v").await.unwrap();
        state.credential_store.update("tenant-1", &c2.id, None, None, Some(Some(now + 365 * 86400))).await.unwrap();
