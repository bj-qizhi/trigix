...
impl MemorySubscriptionStore {
    ...
    ) -> Result<EventSubscription, SubscriptionError> {
...
impl PostgresSubscriptionStore {
    ...
    pub async fn delete(&self, tenant_id: &str, id: &str) -> Result<(), SubscriptionError> {
        let result =
            sqlx::query("DELETE FROM af_event_subscriptions WHERE id = $1 AND tenant_id = $2")
                .bind(id)
                .bind(tenant_id)
                .execute(&self.pool)
                .await
    ...
    pub async fn matching(
        ...
        tenant_id: &str,
        event: &str,
    ) -> Result<Vec<EventSubscription>, SubscriptionError> {
        #[derive(sqlx::FromRow)]
        struct Row {
            id: String,
            tenant_id: String,
...
impl Default for PlatformSubscriptionStore {
    fn default() -> Self {
        Self::Memory(MemorySubscriptionStore::default())
    }
}

impl PlatformSubscriptionStore {
    pub fn postgres(pool: PgPool) -> Self {
        Self::Postgres(PostgresSubscriptionStore::new(pool))
    }
...
pub fn fire_event(
    store: Arc<PlatformSubscriptionStore>,
    tenant_id: String,
    event: &'static str,
...
mod tests {
    ...
    fn store() -> MemorySubscriptionStore {
        MemorySubscriptionStore::default()
    }
