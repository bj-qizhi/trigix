...
use desktop_protocol::{
    DesktopAction, DesktopCommand, DeviceCapability, DeviceConnectionAccepted, DeviceDescriptor,
    DeviceState, Envelope, ExecutionLease, Heartbeat, PairingRequest,
};
        ...
        tokio::select! {
            chunk = response.chunk() => {
                match chunk? {
                    Some(chunk) => {
                        pending.push_str(&String::from_utf8_lossy(&chunk));
                        if parse_sse_data(&mut pending, "disconnect")?.is_some() {
                            return Ok(());
                        }
                    }
...
fn parse_sse_data(
    ...
    while let Some((boundary, delimiter_length)) = buffer
        .find("\r\n\r\n")
        .map(|index| (index, 4))
        .or_else(|| buffer.find("\n\n").map(|index| (index, 2)))
    {
