...
//! FTP node (suppaftp). Plain FTP or explicit FTPS (AUTH TLS) via the `secure`
//! flag. Blocking client driven on a blocking thread so it never stalls the
//! async runtime. Binary payloads cross as base64.
...
// The op methods are identical across the plain and TLS stream types, but the
// concrete types differ and suppaftp's stream trait isn't public — a macro lets
// us run the same operations against whichever stream we opened.
macro_rules! ftp_run_ops {
    ($ftp:ident, $operation:expr, $path:expr, $content_b64:expr) => {{
...
pub(super) async fn execute_ftp(node: &Node, context: &ExecutionContext) -> NodeExecutionResult {
    ...
    let pass = cfg
        ...
        .unwrap_or("")
        .to_string();
    let secure = cfg.get("secure").and_then(|v| v.as_bool()).unwrap_or(false);
    let operation = cfg
        .get("operation")
        .and_then(|v| v.as_str())
        .unwrap_or("list")
    ...
    let result = tokio::task::spawn_blocking(move || -> Result<serde_json::Value, String> {
        ...
        let out = if secure {
            // Explicit FTPS: connect plaintext, then upgrade with AUTH TLS.
            let stream =
                NativeTlsFtpStream::connect(addr).map_err(|e| format!("connect error: {e}"))?;
            ...
            let mut ftp = stream
                ...
                .map_err(|e| format!("TLS upgrade error: {e}"))?;
            ftp.login(&user, &pass)
                .map_err(|e| format!("login error: {e}"))?;
            let out = ftp_run_ops!(ftp, operation, path, content_b64);
            let _ = ftp.quit();
            out
        } else {
            let mut ftp = FtpStream::connect(addr).map_err(|e| format!("connect error: {e}"))?;
            ftp.login(&user, &pass)
                .map_err(|e| format!("login error: {e}"))?;
            let out = ftp_run_ops!(ftp, operation, path, content_b64);
            let _ = ftp.quit();
